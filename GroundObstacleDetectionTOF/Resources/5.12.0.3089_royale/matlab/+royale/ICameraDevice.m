classdef ICameraDevice < handle
    ...
    methods
        ...
        function [DepthData, IntermediateData] = getData(obj)
            switch nargout
                case 0
        ...
        function RawData = getRawData(obj)
            RawData = royale.royale_matlab(obj,'getRawData');
        end
        ...
        function setFilterPreset(obj, filterPreset, streamId)
            if ~isa(filterPreset, 'royale.FilterPreset')
                error('invalid argument');
            end
            if (nargin == 2)
                royale.royale_matlab(obj,...
                    'setFilterPreset', uint16(filterPreset));
            else
                royale.royale_matlab(obj,...
                    'setFilterPreset', uint16(filterPreset), uint16(streamId));
            end
        end
        function filterPreset = getFilterPreset(obj, streamId)
            if (nargin == 1)
                filterPreset = royale.FilterPreset(...
                    royale.royale_matlab(obj,'getFilterPreset'));
            else
                filterPreset = royale.FilterPreset(...
                    royale.royale_matlab(obj,'getFilterPreset', uint16(streamId)));
            end
        end
        ...
        function registerRawDataListener(obj)
            royale.royale_matlab(obj,'registerRawDataListener');
        end
